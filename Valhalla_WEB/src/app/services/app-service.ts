...
export class AppService {
    ...
    return new Promise<Array<any>>((resolve: any, reject) => {
      this.apollo
       ...
       `
          ...
          return resolve(JSON.parse(JSON.stringify(data)).offersacepted);
        }, (error) => {
          return reject(error);
        });
    })
  ...
  listarResenasCantidad() {
    return new Promise<Array<any>>((resolve: any, reject) => {
      ...
      `
          ...
          return resolve(JSON.parse(JSON.stringify(data)).allReviews);
        }, (error) => {
          return reject(error);
        });
    })
  ...
  AgregarCalificacion(id_user: string,ranting:string, fk_oferta:string) {
    return new Promise<Array<any>>((resolve: any, reject) => {
      this.apollo
        .query({
          query: gql`
       query($id_user:String!, $fk_oferta:String!, $ranting:String!){
        AgregarCalificacion(id_user:$id_user, fk_oferta:$fk_oferta,ranting:$ranting)
       }
       `, variables: {
        id_user:id_user,fk_oferta:fk_oferta,ranting:ranting}
        }).subscribe(({ data }) => {
          return resolve(JSON.parse(JSON.stringify(data)).AgregarCalificacion);
        }, (error) => {
          return reject(error);
        });
    })
  ...
  AgregarResena(id_user: string,review:string, fk_oferta:string) {
    return new Promise<Array<any>>((resolve: any, reject) => {
      this.apollo
        .query({
          query: gql`
       query($id_user:String!, $fk_oferta:String!, $review:String!){
        AgregarResena(id_user:$id_user, fk_oferta:$fk_oferta,review:$review)
       }
       `, variables: {
        id_user:id_user,fk_oferta:fk_oferta,review:review}
        }).subscribe(({ data }) => {
          return resolve(JSON.parse(JSON.stringify(data)).AgregarResena);
        }, (error) => {
          return reject(error);
        });
    })
  ...
  modificarEstadoOffertas(fk_offer: string, stado:string) {
    return new Promise<Array<any>>((resolve: any, reject) => {
      this.apollo
        .mutate({
          mutation: gql`
       mutation($fk_offer:String!,$stado:String!){
        updateOffer(state:$stado,id_offer:$fk_offer)
       }
       `, variables: {
            fk_offer: fk_offer,stado:stado
          }
        }).subscribe(({ data }) => {
          return resolve(JSON.parse(JSON.stringify(data)).updateOffer);
        }, (error) => {
          return reject(error);
        });
    })
