...
export class InicioComponent implements OnInit {
      ...
      response.forEach(async element => {
        ...
        this.selectedItems.forEach(async preferencias => {
          if (preferencias.itemName == element.FK_TypeOffer.TypeOffer &&
            (this.fechaInicio != undefined ? new Date(this.fechaInicio.year + '-' + this.fechaInicio.month + '-' + this.fechaInicio.day) : new Date(element.Date)) <= new Date(element.Date) &&
            (this.fechaFinal != undefined ? new Date(this.fechaFinal.year + '-' + this.fechaFinal.month + '-' + this.fechaFinal.day) : new Date(element.Date)) >= new Date(element.Date)) {
            encontrado = 1;
          }
        })
  ...
  cargarResenas(index: number) {
    this.selectOffer = index;
    this.resenasList = this.listaTipoOfertas[index].resena;
  ...
  ListarPorResenas() {
    this.listaTipoOfertas.sort((a, b) => {
      return b.resena.length - a.resena.length;
    });
  }
